...
import { Plus, Trash2, Layers, Tag as TagIcon, Search, Filter, Upload, Image as ImageIcon, X } from 'lucide-react';
...
const Collections = () => {
  ...
  const handleAddCollection = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || error.response?.data?.message || 'Error adding collection');
    } finally {
      setUploading(false);
    }
  };
                ...
                {loading ? (
                  ...
                  collections.map((collection) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-gray-900 dark:text-white  decoration-black/20 decoration-2 underline-offset-4">{collection.name}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs text-gray-500 dark:text-gray-400">{collection.description || '—'}</span>
                      </td>
